...
public class Main {
    public static void main(String[] args) {
        Dog dog1 = new Dog();
        dog1.makeSount();   // 개가 짖습니다.
        dog1.fetch();       // 강아지가 공을 물어옵니다.

        // 업캐스팅 # 1
        Animal animal1 = dog1;
        animal1.makeSount();    // 개가 짖습니다. 즉 , 재정의된 method가 호출됐습니다.
//        animal1.fetch();  // 이게 불가능합니다.

        // 업캐스팅 # 2
        Animal animal2 = new Dog(); // 이게 어제 말한 A a = new B(); 형태입니다. 시험에 자주 나옵니다.
        animal2.makeSount();
//        animal2.fetch();
    /*
        클래스A 객체명 = new 클래스B();를 통해 객체를 생성했을 경우
        1. Override가 적용된 메서드, 즉 자식 클래스의 메서드가 호출됨(클래스 B의 메서드라는 의미겠네요)
...
//        dog3.makeSount();
        /*
            ...
            발생했음을 확인할 수 있습니다.
            즉 애초에 animal3를 생성할 때 Animal클래스의 생성자를 호출해서 만들었기 때문ㅇ
            (A a = new A(); 이기 때문에)
            하위 클래스의 인스턴스로 다운캐스팅하는 것이 불가능합니다.
         */
        Animal animal4 = new Dog();
        // 와 같은 방식으로 선언된 클래스와 호출되는 생성자가 서로 달라야 합니다( 그리고 둘은 '상속'관계여야만 합니다.)
        ...
        if(animal4 instanceof Dog) {        // 이 조건식이 true라면 animal4의 생성시 호출된 생성자는 Dog();라는 의미
            animal4.makeSount();            // 그러므로 animal4.makeSound()를 호출했을 때의 결과값은 개가 짖습니다.
            // 근데 fetch도 호출하고 싶다면
