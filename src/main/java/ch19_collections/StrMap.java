...
public class StrMap {
    ...
    // key - value 삽입 메서드 → .puy(키, 값);
        ...
        koreanScores2.put("김사", 67.95);
        koreanScores2.put("김오", 70.1);
        // 특정 key의 value 수정

        // # 1 아까 한 것처럼 key를 정확하게 쓴 다음에 value를 수정하는것
        ...
        // 그럼 이제 우리 수준에서는 strMap1.entrySet()의 호출 결과가 일종의 형변환이 이루어진 로진이 있고
        // 해당 자료형으로 return 해서 entrySet2라는 객체에 데이터를 담았다고 해석할 수 있겠습니다.
        System.out.println(entrySet2);

        // 근데 Map을 Set으로 바뀠다면 아까 학습한 내용을 토대로 봤을 때 Set → List로 형변환할 수 있네요.
//        List<Map.Entry<String , String >> entryList1 = strMap1.
        // 근데 위에는 보니까 안되네요 Map을 List로 바꾸고 싶다면 Set을 경유하도록 하겠습니다.
...
//        Set<String> valueSet = strMap1.values();    // .values()가 있는거 보니까
        /*
            ...
            각 Key의 Value들은 중복 값이 있을 수도 있기 때문에 그것을 Set자료형으로 변경할 경우
        ...
        */
        ...
        // 로 써보면 되지 않을까 라고 스스로 추측할 수 있어야 한다
        /*
            Collections는 .sort()할 때 써봤지만 Collections은 처음 써봤습니다.
            근데 return 형 본 다음에 직접 쓴다면 좋습니다.
         */
    }
}