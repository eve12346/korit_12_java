...
/*
    배열의 출력
    엄밀히 말하면 저희는 배열을 출력한게 아니라 매열 내부의 element들을 출력
    ...
    이상에서 중요한 점은
    클래스명.메서드명() 이라는 점입니다.
    scanner.nextLine();의 경우에는 객체명.메서드명()이었는데 차이가 있씁니다.
...
public class Array10PrintArray {
    public static void main(String[] args) {
         ...
         */
        // 이상의 경우에서 random.nextInt();는 int 범위 내에서 임의의 정수룰
        // 생성하는 메서드이고, scanner.nextInt();는 int 값을 콘솔에서 입력하는
        // 메서드라는 점을 알 수 있씁니다. 즉, 같은 method명이면서도
        // 어떤 클래스의 객체에서 호출하느냐에 따라서 결과값이 다릅니다.
...
//        System.out.println(Arrays.asList(numbers).toString());
        Method01.call1();   // 그래서 객체명.메어스명(); 할 때와 달리 이때는 메서드 정의할 때 무언가 하나 더 쓰고
                            // 메서드를 정의했었습니다(추후 수업 예정)
        // chainging method로 toString()이 있음을 확인
...
//        System.out.println(Arrays.toString(Arrays.sort(nums)));
        ...
        // 매개변수가 두 개로 늘어났으니까요.
        Arrays.sort(nums, Comparator.reverseOrder());   // 현재 오류가 발샘함 int[]
        ...
        /*
            ...
            메서드의 결과값이 sort()의두 번째 argument로 사용되었다는 점에서
            함수형 프로그래밍의 일종이라고 볼 수 있겠습니다.
            근데 기본 자료형(primitive typse)인 int[] 배열을 사용할 수 없어서
