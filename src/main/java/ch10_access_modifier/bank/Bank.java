...
import ch09_getter_setter.Person;
...
public class Bank {
    ...
    private int balane;             // 잔액, 정수
    private int pinNumber;          // 비밀번호, 4 자리 정서 0 ~ 9999
    ...
    public Bank(int accountNum, String accountHolder, int balane) {
        this.accountNum = accountNum;
        this.accountHolder = accountHolder;
        this.balane = balane;
    }

    public Bank(int accountNum, String accountHolder, int balane, int pinNumber) {
        this.accountNum = accountNum;
        this.accountHolder = accountHolder;
    ...
    public void deposit(int amount, int inputPin) { // 입금 관련 메서드
        if (pinNumber == inputPin) {
            if (amount > 0) {
                balane += amount;
                System.out.println(amount + "원이 입금되었습니다. 현재 잔액 : " + balane);
            } else {
    ...
    public void withdraw(int amount, int inputPin) {// 출금 관련 메서드
        if (pinNumber == inputPin) {
            balane -= amount;
            if (balane < 0) {
                System.out.println("잔액이 부족하여 출금 불가입니다.");
                balane += amount;
            } else {
                System.out.println(amount + "원이 출금되었습니다. 현재 잔액 : " + balane + "원");
            }
        } else {
            System.out.println("비밀번호가 틀렸습니다.");
        }
