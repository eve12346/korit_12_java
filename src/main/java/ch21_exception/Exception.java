...
import ch14_casting.centralcontrol.AirConditioner;
...
//        // 그러면 개발자가 나누는 수를 0으로 입력한 것을 예상할 수 있기 때문에
//        if ( b == 0) {
...
//            System.out.println(result);
//        }
        // 즉 이상의 코드는 아까 확인한 예외와 관려된 클래스를 사용하지 않는 예시라고 할 수 있습니다.
        ...
        try {
            ...
            System.out.println("(finally)얘는 예외가 발생하건 말건 끝에 실행됩니다.");
        }
        /*
            왜 예외 처리를 하는가?
                : 멘토 파이썬에서 봤었는데 프로그램을 실행시킬 때 시뻘겋게 되어있으면
                나쁘기 때문에 예외 처리를 해야 함. 이라고 적혀 있었습니다.
        ...
        try {
            ...
            System.out.println(e.getMessage());
        } catch (java.lang.Exception e) {   // 예외도 서로 상속 처리가 되는데 거의
            throw new RuntimeException(e);  // 가장 상위에 예외 관련 클래스에 해당합니다.
        } finally {
...
/*
    ...
    leapYear 관련할 때 가장 촘촘한 조건은 앞에 위치시킨 것처럼.
 */
